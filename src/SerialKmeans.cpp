...
void SerialKmeans::fit(dataset_t &data, value_t (*func)(datapoint_t &, datapoint_t &))
{
    ...
    for (int run = 0; run < numRestarts; run++)
    {
        ...
        } while (changed > (numData * 0.0001)); // do until 99.9% of data doesnt change
...
void SerialKmeans::scaleableKmeans(dataset_t &data, int &overSampling, value_t (*func)(datapoint_t &, datapoint_t &), int initIters)
{
                                      ...
                                      value_t (*func)(datapoint_t &, datapoint_t &))
{
    ...
    if (minDist < distances[pointIdx] && minDist >= 0)
    {
