...
int main(int argc, char *argv[])
{
    ...
    CReader reader;
    reader.read("../test_100000_15.txt", numData, numFeatures);
    value_t *data = reader.getData();

    DataSetReader dReader;
    reader.read("../test_100000_15.txt", numData, numFeatures);
    dataset_t data2 = dReader.getData();

    RNGType rng(time(NULL));
    boost::uniform_int<> intRange(0, data2.size());
    ...
    if (rank == 0) std::cout << "CORES = " << numProcs << " DATA_SIZE = " << numData << " FEATURES = " << numFeatures << " CLUSTERS = " << numClusters << std::endl;
    ...
    for(int j=0; j < trials; j++)
    {
        ...
        {
            auto start = std::chrono::high_resolution_clock::now();
            serialKmeanskpp.fit(data2, Kmeans::distanceL2);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::seconds>(stop - start);
            std::cout << "Total time sequential: " << duration.count() << std::endl;
            kPPTimes.push_back(duration.count());
            DataSetWriter writer(kmeans.getClusters(), kmeans.getClustering());
            writer.writeClusters("../clusters_mpi_seq.txt");
            writer.writeClustering("../clustering_mpi_seq.txt");
        }
        ...
        // auto make_coreset_duration = std::chrono::duration_cast<std::chrono::seconds>(make_coreset_stop - make_coreset_start);
    ...
    if(rank == 0)
    {
        ...
        // file_name = "../coreset_creation_mpi_times-" + std::to_string(numData) + "-" + std::to_string(numProcs) + ".txt";
        ...
        // file_name = "../coreset_fit_mpi_times-" + std::to_string(numData) + "-" + std::to_string(numProcs) + ".txt";
