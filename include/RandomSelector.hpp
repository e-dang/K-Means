...
{
public:
    virtual std::vector<int> select(const std::vector<value_t>* const weights, const int& sampleSize) = 0;
};
...
{
public:
    std::vector<int> select(const std::vector<value_t>* const weights, const int& sampleSize) override;
};