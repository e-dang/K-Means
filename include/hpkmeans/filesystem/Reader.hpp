...
Matrix<precision, int_size> MPIMatrixReader<precision, int_size>::read(const std::string& filepath,
                                                                       ...
                                                                       const int_size& numFeatures)
{
    MPIDataChunks<int_size> mpiData(numData);
    auto lengths      = mpiData.myLength() * numFeatures;
    MPI_Offset offset = mpiData.myDisplacement() * numFeatures * sizeof(precision);
    MPI_File fh;
    MPI_Status status;

    MPI_File_open(MPI_COMM_WORLD, filepath.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);

    Matrix<precision, int_size> data(mpiData.myLength(), numFeatures, true);
