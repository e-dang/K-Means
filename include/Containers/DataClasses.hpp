...
struct ClusterData
{
    ...
    ClusterData(const int_size& numData, const int_size& numFeatures, const int_size& numClusters) :
        clusters(numClusters, numFeatures),
        clustering(std::vector<int_size>(numData, -1), clusterWeights(std::vector<precision>(numClusters, 0.0)))
    {
    }
...
struct KmeansData
{
    ...
    const std::shared_ptr<IDistanceFunctor> distanceFunc;
    ...
    KmeansData(const Matrix<precision, int_size>* const data, const std::vector<precision>* const weights,
               std::shared_ptr<IDistanceFunctor> distanceFunc, const int& rank, const int_size& totalNumData,
    ...
    void setSqDistances(std::vector<precision>* const sqDistances) { sqDistances = sqDistances; }
...
struct Coreset
{
    ...
    Coreset(const int_size& numData, const int_size& numFeatures) : data(numData, numFeatures), weights(numData) {}
